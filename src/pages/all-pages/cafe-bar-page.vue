<template>
  <card :title="title" :isLoading="isLoading">
    ...
    <template #body>
      <cafe-bar-form v-model="currentLanguage.data"> </cafe-bar-form>
...
import cafeBarForm from '@/components/all-pages/cafe-bar-form.vue'
...
export default {
  components: { card, cafeBarForm, LanguageComponent, checkboxComponent },
  setup () {
  ...
  methods: {
    GetFile (language) {
      this.cloudFirestore.getDocument(language.collection, 'Cafe_Bar').then((document) => {
        if (document) {
