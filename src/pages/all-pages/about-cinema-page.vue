<template>
  <card :title="title" :isLoading="isLoading">
    ...
    <template #body>
      <about-cinema-form v-model="currentLanguage.data"> </about-cinema-form>
...
import aboutCinemaForm from '@/components/all-pages/about-cinema-form.vue'
...
export default {
  components: { card, aboutCinemaForm, LanguageComponent, checkboxComponent },
  setup () {
  ...
  methods: {
    GetFile (language) {
      this.cloudFirestore.getDocument(language.collection, 'About').then((document) => {
        if (document) {
