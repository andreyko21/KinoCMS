<template>
  <card :title="title" :isLoading="isLoading">
    ...
    <template #body>
      <vip-hall-form v-model="currentLanguage.data"> </vip-hall-form>
...
import vipHallForm from '@/components/all-pages/vip-hall-form.vue'
...
export default {
  components: { card, vipHallForm, LanguageComponent, checkboxComponent },
  setup () {
  ...
  methods: {
    GetFile (language) {
      this.cloudFirestore.getDocument(language.collection, 'Vip_Hall').then((document) => {
        if (document) {
